...
- Tempo changes are normally structural (arranged in the Song screen), but the `BPM` FX command can also nudge the tempo temporarily within a pattern — see [Section 8 — FX Commands](#8-fx-commands). Any FX-driven tempo change resets back to the pattern's own BPM at the start of playback or when the pattern loops.
